...
1. `output/for_googflyer.csv` to:  `Google Drive > CDVS > Services > Workshops > [semester] > Planning`
...
2. `output/workshops.csv` to: `Google Drive > CDVS > Services > Workshops > [semester] > Planning`
    ...
    - check dates
3. Then open the output/workshops.csv copy from excel to notepad++  into columns A, B, and C of the `DVS_Workshops_summary_stats` file 
    - in Google Drive: from `GD > CDVS > Workshops > _Assessment > longitudinal`
    - from excel:  columns J to notpad to A ;  H -> H ; A to C ; 
